...
function sanitizeGeminiSchema(node) {
	...
	if ("$ref" in node) throw new InvalidInputError("Gemini does not support recursive or referenced ($ref) schemas in function or response schemas. Inline the schema instead.");
	const source = node;
	const schema = {};
...
function schemaToGeminiParameters(schema) {
	return sanitizeGeminiSchema(isInteropZodSchema(schema) || isSerializableSchema(schema) ? toJsonSchema(schema) : schema);
}
/**
...
//#endregion
export { convertToolChoiceToGeminiConfig, convertToolsToGeminiFunctionDeclarations, convertToolsToGeminiTools, mixesBuiltinAndFunctionTools, schemaToGeminiParameters };
