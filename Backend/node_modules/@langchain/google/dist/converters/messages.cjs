...
function convertStandardContentMessageToGeminiContent(message, messages) {
	...
	if (_langchain_core_messages.AIMessage.isInstance(message) && message.tool_calls?.length) {
		...
		for (const block of contentBlocks) if (block.type === "tool_call" && block.id && "thoughtSignature" in block && typeof block.thoughtSignature === "string") signaturesById[block.id] = block.thoughtSignature;
		for (const toolCall of message.tool_calls) {
			const part = { functionCall: {
				name: toolCall.name,
				args: toolCall.args ?? {}
			} };
			const thoughtSignature = (toolCall.id && signaturesById[toolCall.id]) ?? ("thoughtSignature" in toolCall ? toolCall.thoughtSignature : void 0);
