...
import { convertToolChoiceToGeminiConfig, convertToolsToGeminiTools, mixesBuiltinAndFunctionTools, schemaToGeminiParameters } from "../converters/tools.js";
...
var BaseChatGoogle = class extends BaseChatModel {
	...
	constructor(params) {
		super(params);
		this.params = params;
		this._addVersion("@langchain/google", "0.2.6");
	...
	get endpoint() {
		...
		else if (!this.location.includes("-")) return `aiplatform.${this.location}.rep.googleapis.com`;
	...
	*/
	...
	invocationParams(options) {
		...
		let toolConfig = convertToolChoiceToGeminiConfig(options.tool_choice, !!(tools && tools.length > 0));
		if (tools && mixesBuiltinAndFunctionTools(tools)) toolConfig = {
			...toolConfig,
				...
				if (parts.length > 0 || candidate.finishReason) {
					...
					controller.enqueue(new ChatGenerationChunk({
						...
						generationInfo: {
							...
							...candidate.citationMetadata && { citationMetadata: candidate.citationMetadata },
							...candidate.groundingMetadata && {
								groundingMetadata: candidate.groundingMetadata,
								groundingSupport: candidate.groundingMetadata.groundingSupports?.filter((s) => (s?.segment?.partIndex ?? 0) === 0)
							}
						}
					}));
				}
