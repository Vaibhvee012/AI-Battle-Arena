...
const GEMINI_SCHEMA_KEYS = /* @__PURE__ */ new Set([
	"anyOf",
	"default",
	"description",
	"enum",
...
function sanitizeGeminiSchema(node) {
	...
	if ("$ref" in node) throw new require_errors.InvalidInputError("Gemini does not support recursive or referenced ($ref) schemas in function or response schemas. Inline the schema instead.");
	const source = node;
	const schema = {};
	for (const key of Object.keys(source)) if (GEMINI_SCHEMA_KEYS.has(key)) schema[key] = source[key];
	adjustObjectType(schema);
	if (schema.properties && typeof schema.properties === "object") schema.properties = Object.fromEntries(Object.entries(schema.properties).map(([name, value]) => [name, sanitizeGeminiSchema(value)]));
	if (schema.items !== void 0) schema.items = sanitizeGeminiSchema(schema.items);
	if (Array.isArray(schema.anyOf)) schema.anyOf = schema.anyOf.map(sanitizeGeminiSchema);
	return schema;
}
/**
...
function schemaToGeminiParameters(schema) {
	return sanitizeGeminiSchema((0, _langchain_core_utils_types.isInteropZodSchema)(schema) || (0, _langchain_core_utils_standard_schema.isSerializableSchema)(schema) ? (0, _langchain_core_utils_json_schema.toJsonSchema)(schema) : schema);
}
/**
...
*/
function mixesBuiltinAndFunctionTools(tools) {
	const isFunctionDeclarationTool = (tool) => "functionDeclarations" in tool && !!tool.functionDeclarations?.length;
	return tools.some(isFunctionDeclarationTool) && tools.some((tool) => !isFunctionDeclarationTool(tool));
}
//#endregion
...
exports.mixesBuiltinAndFunctionTools = mixesBuiltinAndFunctionTools;
